...
def generate_tts(text: str, language: str, speaker_file):
    ...
    try:
        ...
        # TTS-Modell laden
        config = XttsConfig()
        config.load_json(os.path.join(TTS.get_model_path(tts_model_name), "config.json"))
        model = Xtts.init_from_config(config)
        model.load_checkpoint(config, checkpoint_dir=TTS.get_model_path(tts_model_name), eval=True, use_deepspeed=False)
...
def generate_tts(text: str, language: str, speaker_file):
    ...
    try:
        ...
        sample_rate = 24000 
        write_wav(str(output_path), sample_rate, np.array(wav)) # Geändert
